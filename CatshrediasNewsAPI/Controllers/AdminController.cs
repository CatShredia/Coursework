...
{
    private bool IsRssSourcesLockedInProd() => env.IsProduction();
    ...
    // ? ExportSourcesSql : выгружает SQL-скрипт c текущим содержимым RssSources
    // вызывается клиентом (Admin)
    [HttpGet("sources/export-sql")]
    public async Task<IActionResult> ExportSourcesSql()
    {
        if (IsRssSourcesLockedInProd())
            return StatusCode(StatusCodes.Status403Forbidden, "В Production выгрузка RssSources отключена.");

        var sources = await rssSourceService.GetAllAsync();
        var sb = new System.Text.StringBuilder();
        sb.AppendLine("-- Generated by Runews admin export");
        sb.AppendLine("UPDATE \"RssSources\" SET \"IsEnabled\" = false;");
        sb.AppendLine();
        ...
        foreach (var s in sources.OrderBy(x => x.Id))
        {
            ...
            var sourceType = string.Equals(s.SourceType, "Scraper", StringComparison.OrdinalIgnoreCase) ? 1 : 0;
            sb.AppendLine(
                $"INSERT INTO \"RssSources\" (\"Id\",\"Name\",\"Url\",\"IsTrusted\",\"IsEnabled\",\"SourceType\",\"LinkSelector\",\"TitleSelector\",\"ContentSelector\",\"DateSelector\",\"ImageSelector\") VALUES " +
                $"({s.Id}, {Sql(s.Name)}, {Sql(s.Url)}, {(s.IsTrusted ? "true" : "false")}, {(s.IsEnabled ? "true" : "false")}, {sourceType}, {Sql(s.LinkSelector)}, {Sql(s.TitleSelector)}, {Sql(s.ContentSelector)}, {Sql(s.DateSelector)}, {Sql(s.ImageSelector)}) " +
                "ON CONFLICT (\"Id\") DO UPDATE SET " +
                "\"Name\" = EXCLUDED.\"Name\", \"Url\" = EXCLUDED.\"Url\", \"IsTrusted\" = EXCLUDED.\"IsTrusted\", \"IsEnabled\" = EXCLUDED.\"IsEnabled\", " +
                "\"SourceType\" = EXCLUDED.\"SourceType\", \"LinkSelector\" = EXCLUDED.\"LinkSelector\", \"TitleSelector\" = EXCLUDED.\"TitleSelector\", " +
                "\"ContentSelector\" = EXCLUDED.\"ContentSelector\", \"DateSelector\" = EXCLUDED.\"DateSelector\", \"ImageSelector\" = EXCLUDED.\"ImageSelector\";");
        }

        var bytes = System.Text.Encoding.UTF8.GetBytes(sb.ToString());
        return File(bytes, "application/sql", $"rss-sources-{DateTime.UtcNow:yyyyMMdd-HHmmss}.sql");
    }
    ...
    {
        if (IsRssSourcesLockedInProd())
            return StatusCode(StatusCodes.Status403Forbidden, "В Production изменение RssSources отключено.");
